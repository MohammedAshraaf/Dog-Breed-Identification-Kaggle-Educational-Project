...
class NeuralNetwork:
    ...
    def __init__(self, X, Y, layers_dims, learning_rate=0.01, num_iterations=2000, print_cost=False):
        """
    ...
    def compute_cost(self, AL, Y):
        ...
        Y -- true "label" vector,
        shape (number of classses, number of examples)
    ...
    def deep_model_backward(self, AL, caches):
        """
        ...
        dAL = - (np.divide (self.Y, AL) - np.divide (1 - self.Y, 1 - AL))  # derivative of cost with respect to AL
    ...
    def train(self, X_val=None, Y_val=None):
        ...
        for i in range(0, self.num_iterations):
            ...
            grads = self.deep_model_backward(AL, caches)
            ...
            if X_val is not None and Y_val is not None and i % 10 == 0:
                ...
                if val_cost < min_cost:
                    min_cost = val_cost
                    print("model has lower loss on validation set %f" % val_cost)
                    self.val_param = self.parameters
